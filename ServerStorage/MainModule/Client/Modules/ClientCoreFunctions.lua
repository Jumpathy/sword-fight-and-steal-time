...
function module:UpdateIconVisiblity()
	local enabled = false
	if main.pdata.Rank >= main.settings.RankRequiredToViewIcon then
		enabled = true
	end
	main:GetModule("TopbarIcon"):setEnabled(enabled)
...
function module:CreateNewCommandMenu(commandName, menuDetails, menuType, forceOnTop)
	...
	if not frame then
		...
		elseif(menuType == 13) then
			...
			mainFrame.Parent.DragBar.Title.Text = "1V1";
			local props = {
				maxSuggestions = 3;
				mainParent = mainFrame.Parent;
				textBox = playerTextBox;
			...
			banButton.MouseButton1Down:Connect(function()
				...
				local s,e = pcall(function()
					game.ReplicatedStorage.request1v1:FireServer(game:GetService("Players"):GetUserIdFromNameAsync(playerName));
				end)
