...
function module:SetupCommands(commands, page, totalLabels)
	local yBound = templateC1.TextLabel.TextBounds.Y
	for i, command in pairs(commands) do
		...
		if page.Name == "Donor" then
			...
			labelId = main:GetModule("cf"):GenerateTagFromId(totalLabels)
		end
		...
		local textBounds = main.textService:GetTextSize(
		    textLabel.Text,
		    yBound,
		    textLabel.Font,
		...
		if command.SpecialColor then
			textLabel.Text = command.OriginalName
			local newTextBounds = main.textService:GetTextSize(
			    textLabel.Text,
			  	yBound,
			    textLabel.Font,
			    textLabel.AbsoluteSize
			...
			colorLabel.ZIndex = colorLabel.ZIndex + 2
			colorLabel.Position = textLabel.Position + UDim2.new(0, newTextBounds.X, 0, 0)
			colorLabel.TextColor3 = command.SpecialColor
			colorLabel.Name = "ColorLabel"
			colorLabel.Text = command.SpecialColorName
			colorLabel.Parent = label
		...
		detailsLabel.Position = textLabel.Position + UDim2.new(0, textBounds.X+5, 0, 0)
		...
		detailsLabel.Text = table.concat(newDetail, "  ")
		detailsLabel.Parent = label
		------------
		label.Name = labelId
		label.Arrow.Text = "►"
		label.Visible = true
...
function module:CreateCommands()
	if not creatingCommands then
		...
		main:GetModule("cf"):ClearPage(pages.commands)
		...
		for _, rankGroupDetails in pairs(ranks) do
			...
			if #commands > 0 and (not main.settings.OnlyShowUsableCommands or main.pdata.Rank >= rankId) then
				...
				totalLabels = totalLabels + 1
				rankLabel.Name = main:GetModule("cf"):GenerateTagFromId(totalLabels)
...
-- << MORPHS >>
function module:CreateVoiceChat()
	--[[
	main:GetModule("cf"):ClearPage(pages.voiceChat)
	...
	for morphName,_ in pairs(main.morphNames) do
		...
		if string.match(cName, searchText) then
			table.insert(voiceChatList, morphName)
		end
	end
