...
local success, systemDataStore = pcall(function() return(main.dataStoreService:GetDataStore("HDAdminSystemData"..DataVersion)) end)
...
coroutine.wrap(function()
	...
	while true do
		...
		for i,key in pairs(keys) do
			...
			local success, data = main:GetModule("DataStores"):GetData(systemDataStore, datastoreRetries, key)
				...
				for statName, defaultStatValue in pairs(dataTemplate) do
					...
					if statName == "Records" then
						for i, record in pairs(statValue) do
							...
							coroutine.wrap(function()
								record.Name = main:GetModule("cf"):GetName(record.UserId)
							end)()
						end
