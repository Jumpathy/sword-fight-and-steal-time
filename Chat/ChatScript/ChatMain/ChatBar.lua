...
function methods:SetUpTextBoxEvents(TextBox, TextLabel, MessageModeTextButton)
	...
	local fire = function(bool)
		if(last ~= bool) then
			last = bool;
			game.ReplicatedStorage.events.typingIndicator:FireServer(bool);
		end
	end
	...
	local handle = function()
		lastKey = tick();
		local current = lastKey;
		fire(true);
		coroutine.wrap(function()
			wait(5);
			if(TextBox:IsFocused() and lastKey == current) then
				fire(false);
			end
		end)();
	end
