...
EventFolder.OnMessageDoneFiltering.OnClientEvent:connect(function(messageData)
	...
	coroutine.wrap(function()
		local s,player = pcall(function()
			return game:GetService("Players"):GetPlayerByUserId(messageData.SpeakerUserId or 0);
		end)
		shared.fire_event(messageData,player);
	end)();
...
function HandleChannelJoined(channel, welcomeMessage, messageLog, channelNameColor, addHistoryToGeneralChannel,
	...
	if (channelObj) then
		...
		game.ReplicatedStorage.events.makeMsg.OnClientEvent:Connect(function(m,n)
			local welcomeMessageObject = {
				ID = -1,
				FromSpeaker = nil,
				SpeakerUserId = 0,
				OriginalChannel = channel,
				IsFiltered = true,
				MessageLength = string.len(m),
				MessageLengthUtf8 = utf8.len(utf8.nfcnormalize(m)),
				Message = trimTrailingSpaces("["..(n or "System").."]: " .. m),
				MessageType = ChatConstants.MessageTypeWelcome,
				Time = os.time(),
				ExtraData = nil,
			}
