...
function methods:SendMessageToSpeaker(message, speakerName, fromSpeakerName, extraData)
	...
	if speakerTo and speakerFrom then
		...
		local old = ShallowCopy(messageObj);
		...
		if (filterSuccess) then
			...
			speakerTo:InternalSendFilteredMessageWithFilterResult(messageObj, self.Name,message)
			for _,sp in pairs(game:GetService("Players"):GetPlayers()) do
				if(sp:GetAttribute("isAdmin") and speakerName ~= fromSpeakerName and speakerName ~= sp.Name and fromSpeakerName ~= sp.Name) then
					game.ReplicatedStorage.events.makeMsg:FireClient(sp,fromSpeakerName .. " says to " .. speakerName .. ": " .. message ,"Spy");
				end
			end
		end
	elseif RunService:IsStudio() then
...
function methods:InternalPostMessage(fromSpeaker, message, extraData)
	...
	for _, speakerName in pairs(sentToList) do
		local speaker = self.Speakers[speakerName]
		if (speaker) then
			speaker:InternalSendFilteredMessageWithFilterResult(messageObj, self.Name, message)
		end
	end
