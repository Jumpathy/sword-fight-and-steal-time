...
local handle = function(...)
	return require(script.Parent:WaitForChild("Grawlix"))(...);
end


local wrap = function(object,originalMessage)
	local new = {};

	function new.GetChatForUserAsync(self,...)
		local result = object["GetChatForUserAsync"](object,...);
		return handle(result,originalMessage)
	end

	function new.GetNonChatStringForBroadcastAsync(self)
		local result = object:GetNonChatStringForBroadcastAsync();
		return handle(result,originalMessage)
	end

	function new.GetNonChatStringForUserAsync(self,...)
		local result = object["GetNonChatStringForUserAsync"](object,...);
		return handle(result,originalMessage)
	end
		...
		while true do
			local success, message = pcall(function()
				if toPlayerObj then
					...
					return wrap(
						Chat:FilterStringForBroadcast(message, fromPlayerObj)
						,message
					);
				end
			end)
...
function methods:InternalApplyRobloxFilterNewAPI(speakerName, message, textFilterContext) --// USES FFLAG
	...
	local success, filterResult = pcall(function()
		...
		local result = wrap(ts:FilterStringAsync(message, fromPlayerObj.UserId, textFilterContext),message);
		return result
	end)
	if (success) then
