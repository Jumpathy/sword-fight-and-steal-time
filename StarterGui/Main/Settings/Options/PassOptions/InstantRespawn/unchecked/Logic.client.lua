...
local clicked = function()
	...
	coroutine.wrap(function()
		game.ReplicatedStorage.events.instantRespawn:FireServer(state);
		game.ReplicatedStorage.events.setSetting:FireServer("instantRespawn",state);
	end)();
...
game.ReplicatedStorage:WaitForChild("events"):WaitForChild("getOptions").OnClientEvent:Connect(function(data)
	if(data.instantRespawn) then
		if(not state) then
			clicked();
		end
	end
end)
game.ReplicatedStorage:WaitForChild("events"):WaitForChild("getOptions"):FireServer();